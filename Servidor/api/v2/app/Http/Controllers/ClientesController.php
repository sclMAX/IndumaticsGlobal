...
class ClientesController extends Controller
{
	...
	public function update(Request $request)
	{
		...
		$idSucursal = DB::table('usuarios')->where('idSesion', '=', $idSesion)->value('idSucursal');
		if ($idSucursal) {
			$sCliente = $request->input('Cliente');
			...
			DB::table('clientes')->where('idCliente', $idCliente)->update($newCliente);
			$cliente = DB::select("SELECT * FROM clientes WHERE (idSucursal = $idSucursal) AND (idCliente = $idCliente)");
			$response = new Response($cliente, 200);
		}
		else {
			$response = new Response("Accesso Denegado", 401);
